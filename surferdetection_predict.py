...
Usage:
predict() returns a 1D numpy array of positive identifications. It 
accepts a directory path to a set of test images that have already been 
"tiled", that is broken up into 80x80 patches. predict() can also return 
positive identifications if and only if the softmax for a positive 
identification exceeds a passed threshold.
"""
...
def predict(test_directory, threshold, prob = False):
  ...
  with tf.Graph().as_default() as g:
    ...
    tiles = [image for image in natural_sort(os.listdir(test_directory)) if not image.startswith('.')]
    for tile in range(len(tiles)): tiles[tile] = os.path.join(test_directory, tiles[tile])
    num_tiles = len(tiles)

    #make a queue from the tile names
    tiles_queue = tf.train.string_input_producer(tiles, shuffle = False) #  list of tiles to read
    ...
    variable_averages = tf.train.ExponentialMovingAverage(surferdetection.MOVING_AVERAGE_DECAY)
    ...
    with tf.Session() as sess:
      ...
      while step < num_iter and not coord.should_stop():
        ...
        # if confidence in prediction exceeds threshold, return 1 for positive idenitication
        if prob  == True:
          probabilities = sess.run(tf.nn.softmax(logit))
          
          if np.argmax(probabilities, axis=1) == 1: 
            prediction_map[step] = 1 if np.amax(probabilities, axis=1) >= threshold else 0
