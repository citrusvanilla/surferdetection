...
Usage:
augment() combines all individual distortions and operates on one image at a
time. Please note that the individual distortion functions are not commutative.
...
def color_temp(image):
    ...
    """
    # Pull random "warmth" from a pre-defined kelvin table.
...
def rotate(img_as_array, max_rotation_angle):
    """Randomly rotates image and resizes image to the extent of the rotated
       image so that no pixels are lost.
       Please note this function is not commutative.  See augment() function.

      Args:
        img_as_array: image as 3D numpy array.
        max_rotation_angle: int or float indicating the maximum absolute angle
                            of rotation.
...
def resize(img_as_array,output_size):
    ...
    return transform.resize(img_as_array, output_size, order=1,
                            mode='constant', cval=140, clip=True,
...
def augment(input, random_a = 0.75,
            max_rotation_angle=12,
            min_xy_scale_factor=0.75,
            output_size = (80,80),
            rand_hor_flip = True,
            rand_color_temp = False,
            rand_contr_bright = False):
    """Combines all augmentation operations.  Subfunctions are not commutative
       due to the nature of the SURFERDETECTION dataset.  See README for more
       information.

      Args:
        input: image as NUMPY array from main or sample image file for
               debugging
        random: random percentage to perform augmentation
        max_rotation_angle: maximum accepted angle of image rotation, as int
                            or float
        min_xy_scale_factor: mimimum accepted rescaling factor for image x and
                             y, as float
      ...
      Returns:
        ...
        distortion_stats: statistics of the augmentation routine as a dict
                          (debugging)
    """
    ...
    if random.random() < random_a:
        ...
        # Ramdon contrast and brightness:
        ...
        # Resize image so that no information is lost:
        if rotation_angle != 0:
            img = resize(img,output_size)
        ...
        # Returns a dict of stats for debugging.
        distortion_stats = {
